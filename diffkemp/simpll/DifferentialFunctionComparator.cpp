...
bool DifferentialFunctionComparator::maySkipCast(const User *Cast) const {
    ...
    if (auto StrTy = dyn_cast<StructType>(SrcTy)) {
        if (StrTy->hasName() && refHasPrefix(StrTy->getName(), "union")) {
...
bool DifferentialFunctionComparator::cmpCallArgumentUsingCSource(
    ...
    if (refHasPrefix(CFL->getName(), SimpllInlineAsmPrefix))
        CArgsL =
    ...
    if (refHasPrefix(CFR->getName(), SimpllInlineAsmPrefix))
        CArgsR =
    ...
    if ((CArgsL.size() > i) && (CArgsR.size() > i)) {
        ...
        if (refHasPrefix(StringRef(CArgsL[i]), "sizeof")
            && refHasPrefix(StringRef(CArgsR[i]), "sizeof")
...
std::vector<std::unique_ptr<SyntaxDifference>>
    ...
    if (!refHasPrefix(FunL->getName(), SimpllInlineAsmPrefix)
        || !refHasPrefix(FunR->getName(), SimpllInlineAsmPrefix))
...
void DifferentialFunctionComparator::findTypeDifference(
    ...
    if (cmpTypes(L, R)) {
        ...
        diff->name = refHasPrefix(L->getName(), "struct.")
...
bool DifferentialFunctionComparator::isUnionTy(const Type *Ty) {
    if (auto StrTy = dyn_cast<StructType>(Ty)) {
        return refHasPrefix(StrTy->getStructName(), "union");
    }
    return false;
}
