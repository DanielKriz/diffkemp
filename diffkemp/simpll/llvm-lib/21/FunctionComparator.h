...
protected:
  ...
  LLVM_ABI virtual int compareSignature() const;

  /// Test whether two basic blocks have equivalent behaviour.
  LLVM_ABI virtual int cmpBasicBlocks(const BasicBlock *BBL,
  ...
  LLVM_ABI virtual int cmpConstants(const Constant *L, const Constant *R) const;

  /// Compares two global values by number. Uses the GlobalNumbersState to
  /// identify the same gobals across function calls.
  LLVM_ABI virtual int cmpGlobalValues(GlobalValue *L, GlobalValue *R) const;
  ...
  LLVM_ABI virtual int cmpValues(const Value *L, const Value *R) const;
  ...
  LLVM_ABI virtual int cmpOperations(const Instruction *L, const Instruction *R,
  ...
  LLVM_ABI virtual int cmpTypes(Type *TyL, Type *TyR) const;

  LLVM_ABI virtual int cmpNumbers(uint64_t L, uint64_t R) const;
  LLVM_ABI virtual int cmpAligns(Align L, Align R) const;
  LLVM_ABI virtual int cmpAPInts(const APInt &L, const APInt &R) const;
  LLVM_ABI virtual int cmpConstantRanges(const ConstantRange &L,
                                  const ConstantRange &R) const;
  LLVM_ABI virtual int cmpAPFloats(const APFloat &L, const APFloat &R) const;
  LLVM_ABI virtual int cmpMem(StringRef L, StringRef R) const;
  ...
  LLVM_ABI virtual int cmpGEPs(const GEPOperator *GEPL, const GEPOperator *GEPR) const;
